...
exports.TfClassification = void 0;
...
class TfClassification {
    ...
    async formatEmbeddingsData(data, createModel) {
        const currentUse = await this.loadUse();
        const objectKeys = Object.keys(data);
        const totalTerms = [];
        let totalEmbeddings = {};
        let timeout = 0;
        objectKeys.forEach(async (niceClass, index) => {
            setTimeout(async () => {
                const termArray = data[niceClass].terms.map((term) => {
                    return {
                        text: term,
                        niceClass: data[niceClass].class,
                    };
                });
    ...
    async createModel(data, totalTerms) {
        const N_CLASSES = 45;
        // Define a model for linear regression.
        const model = tf.sequential();
        const xTrain = data;
        const yTrain = tf.tensor2d(totalTerms.map((t) => [
            t.niceClass === 1 ? 1 : 0,
            t.niceClass === 2 ? 1 : 0,
            t.niceClass === 3 ? 1 : 0,
            t.niceClass === 4 ? 1 : 0,
    ...
    async trainSequentialClassifier(niceClasses /* , word: string */) {
        const currentUse = await this.loadUse();
    ...
    async classifyProtection(word, matchedClasses) {
        const currentUse = await this.loadUse();
        const model = await tf.loadLayersModel("file://./src/utils/models/niceClassificationModel/model.json");
        ...
        console.log(`Prediction 0:${prediction[0]};
                Prediction 25: ${prediction[24]};
                Prediction 43: ${prediction[42]}`);
        prediction.forEach((matchingRate, index) => {
            console.log(index + 1);
...
exports.TfClassification = TfClassification;
